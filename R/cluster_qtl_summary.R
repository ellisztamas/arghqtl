...
#' cluster_qtl.
#' @param qtl_labels Optional prefix label for QTL. Defaults to "QTL".
#' 
...
#' 
#' @export
cluster_qtl_summary <- function(qtl_clusters, qtl_labels = "QTL"){
  ...
  summ <- data.frame(chr         = tapply(qtl_clusters$chr, qtl_clusters$QTL_id, mean),
                     ...
                     pos_min     = tapply(qtl_clusters$min_bayesint, qtl_clusters$QTL_id, min),
                     pos_max     = tapply(qtl_clusters$max_bayesint, qtl_clusters$QTL_id, max),
  ...
  rownames(summ) <- paste(qtl_labels, summ$chr,":", unlist(sapply(table(summ$chr), seq)), sep="") # add labels for each QTL.
  return(summ)
}